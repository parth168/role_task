        ...
        .then(async (user) => {
            if (user) {
                ...
                let result = await dao.saveNewUser(newUser)
                    ...
                    .catch((e) => {
                        console.log('>>>>>>>>>> Create-User(Save-New-USer) >>>>>>>>>>>>>>>>>>>',{ e });
...
async function getUSerByRole(req) {
    ...
    if (error_Token) {
        ...
        let user = await dao.findOne({ _id: token._id })
            ...
            .catch((e) => {
                console.log('>>>>>>>>>> Get-User-By-Role(findone query) >>>>>>>>>>>>>>>>>>>',{ e })
                return response.internalServerError();
            });
        if (user.code) {
            return user;
        ...
        return await dao.find(query,'-password',skip,limit,search,sort)
            .then((users) => {
                if (users[0]) {
                    ...
                    .catch((e)=>{
                        console.log('>>>>>>>>>> Get-User-By-Role(Count-Document query) >>>>>>>>>>>>>>>>>>>',{ e })
                        return response.internalServerError();
                    });
                }
                else {
            ...
            .catch((e) => {
                console.log('>>>>>>>>>> Get-User-By-Role(find query) >>>>>>>>>>>>>>>>>>>',{ e })
                return response.internalServerError();
            });
    }
}
...
async function updateUser(req) {
    ...
    if (error_Token) {
        ...
        return await dao.findOne({ _id: req.params.id })
            ...
            .catch((e) => {
                console.log('>>>>>>>>>> Update User(find query) >>>>>>>>>>>>>>>>>>>',{ e });
                return response.internalServerError();
            });
    }
}
...
async function blockUser(req) {
    ...
    if (error_Token) {
        ...
        return await dao.findOne({ _id: req.params.id })
            ...
            .catch((e) => {
                console.log('>>>>>>>>>> Block User(find Query) >>>>>>>>>>>>>>>>>>>',{ e });
                return response.internalServerError();
            });
    }
}
